...
%c(1)=bitsll(uint32(n1),4); % bitsll() requires Fixed Point Designer
c(1) = bitshift(uint32(n1),4);
%c(1)=c(1)+ bitand(15,bitsrl(uint32(m1),18));
c(1)=c(1)+ bitand(15,bitshift(uint32(m1),-18));
%c(2)=bitsll(uint32(m1),6);
c(2)=bitshift(uint32(m1),6);
...
X(2,:)=[x{2},zeros(1,m)];
H=[h,zeros(1,(n+m))]; 
%% create a temporary m-length register TH for h data stream
TH=zeros(1,m);
